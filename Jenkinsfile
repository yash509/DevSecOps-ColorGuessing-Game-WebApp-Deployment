pipeline {
	...
	VAULT_URL = 'http://13.233.207.160:8200/'
    }
	...
	stage("Debugging HashiCorp Vault Credentials") {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-ColorGuessing-Game-WebApp-Deployment.git'
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop clrguesgm | true"
                sh "docker rm clrguesgm | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name clrguesgm -p 5000:80 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service clrguesgm-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"clrguesgm\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
